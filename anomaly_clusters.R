...
require(tidyverse)
{
 ## Read in the anomalous data.
  ...
  anomalous_data <- read.csv(paste0(current_dir,"/Anomalous_Emissions_Results/Anomalous_Emissions_SIBaR.csv"),
                             row.names = 1)
  ...
  # 
  # random_subsets <- lapply(anom_list,function(x) dplyr::sample_n(x,390000))
  # 
...
{
  anom_kmeans <- kmeans(anomalous_emissions,centers = 5, nstart = 100)
